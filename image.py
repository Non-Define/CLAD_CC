...
if __name__ == '__main__':
    ...
    # TODO
    # ----------------------------------------------------
    INPUT_AUDIO_DIR = '/home/cnrl/Workspace/ND/Dataset/asvspoof_5/flac_E_eval'   # .flac
    OUTPUT_IMAGE_DIR = '/home/cnrl/Workspace/ND/Dataset/asvspoof_5/image/lps_E_eval'       # .png
    # ------------------------------------------------------------------------
...
def create_spectrogram(y, sr=SR):
    ...
    return magnitude_spectrogram, status 

def save_spectrogram_image(spectrogram_data, output_filepath):
...
def process_audio_directory(input_dir, output_dir):
    ...
    for audio_path in audio_files:
        filename_base = os.path.splitext(os.path.basename(audio_path))[0]
        output_filepath = os.path.join(output_dir, f'{filename_base}_spec.png')
        
        try:
            y, sr = librosa.load(audio_path, sr=SR)
            spectrogram_data, status = create_spectrogram(y, sr)
            save_spectrogram_image(spectrogram_data, output_filepath)
            print(f"성공: {filename_base}.flac -> {status} -> {output_filepath}")
