...
class Dataset_ASVspoof2019_train(Dataset):
    def __init__(self, list_IDs, labels, utt2spk, base_dir, cut_length=64600, sample_ratio=0.01):
        '''self.list_IDs    : list of strings (each string: utt key),
           self.labels      : dictionary (key: utt key, value: label integer)'''
        
        # 1%만 샘플링된 list_IDs
        self.list_IDs = random.sample(list_IDs, max(1, int(len(list_IDs) * sample_ratio)))  # 1% 샘플링
        self.labels = labels
    ...
    def __getitem__(self, index):
        ...
        X_pad = pad(X, self.cut)  # 데이터가 64600 샘플을 넘지 않도록 패딩
        x_inp = torch.Tensor(X_pad)
        x_inp = torch.unsqueeze(x_inp, 0)  # 채널을 맞추기 위해 차원을 추가
        y = self.labels[key]  # 레이블 가져오기
        spk = self.utt2spk[key]  # 스피커 정보 가져오기
        return x_inp, spk, y
